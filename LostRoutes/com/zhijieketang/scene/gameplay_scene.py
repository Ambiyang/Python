...
from com.zhijieketang.scene import gameover_scene
...
class GamePlayLayer(cocos.layer.Layer):
    ...
    def on_mouse_drag(self, x, y, dx, dy, buttons, modifiers):
        ...
        if self.pause_status == 0:  # 暂停状态不可以移动
    ...
    def test_collision(self):
        ...
        for node in self.get_children():
            ...
            if isinstance(node, Bullet) and node.visible:
                ...
                if bulletrect.intersect(self.stone.get_rect()):
                    self.handle_bullet_enemy_collision(self.stone)
                    node.visible = False
                    continue

                if bulletrect.intersect(self.enemyfighter1.get_rect()):
                    self.handle_bullet_enemy_collision(self.enemyfighter1)
                    node.visible = False
                    continue

                if bulletrect.intersect(self.enemyfighter2.get_rect()):
                    self.handle_bullet_enemy_collision(self.enemyfighter2)
                    node.visible = False
                    continue

                if bulletrect.intersect(self.planet.get_rect()):
        ...
        if self.player.visible:
            ...
            if playerrect.intersect(self.stone.get_rect()):
                self.handle_player_enemy_collision(self.stone)
                return

            if playerrect.intersect(self.enemyfighter1.get_rect()):
                self.handle_player_enemy_collision(self.enemyfighter1)
                return

            if playerrect.intersect(self.enemyfighter2.get_rect()):
                self.handle_player_enemy_collision(self.enemyfighter2)
                return

            if playerrect.intersect(self.planet.get_rect()):
    ...
    def handle_player_enemy_collision(self, enemy):
        ...
        if self.player.hit_points <= 0:
            ...
            ac1 = cocos.actions.Place((self.s_width // 2, 70))
